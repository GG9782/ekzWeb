...
public class UserController {
    ...
    @DeleteMapping("/{email}")
    public ResponseEntity<String> deleteUserByEmail(@PathVariable String email) throws IOException {
        // checkRole("admin")
        Subject subject = SecurityUtils.getSubject();
        subject.checkRole("admin");
        service.removeById(email);

        String img = service.getById(email).getImg();
        Path pathToDelete = Paths.get(PATH.toString(),email + "."+ img);
    ...
    private static final Path PATH = Paths.get("\\\\10.41.34.24\\e\\img\\user");
    ...
    public ResponseEntity<String> uploadImg(@RequestParam("file") MultipartFile file,@PathVariable String email) {
        ...
        try {
            ...
            Path path = Paths.get(PATH.toString(), email + "." + fileExtension);
            ...
            service.lambdaUpdate().eq(User::getEmail,email).set(User::getImg,fileExtension).update();
    ...
    public ResponseEntity<Resource> getPrincipalsImage() {
        ...
        String img = service.getById(principals).getImg();
        if(img == null){return null;}
        try {
            Path path = Paths.get(PATH.toString(),principals + "."+ img);
    ...
    public ResponseEntity<Resource> getImageByEmail(@PathVariable String email) {
        ...
        if(img == null){return null;}
        try {
            Path path = Paths.get(PATH.toString(),email + "."+ img);;
            Resource resource = new UrlResource(path.toUri());
            return ResponseEntity.ok()
                    .header(HttpHeaders.CONTENT_TYPE, "image/png") // 设置图片格式
                    .body(resource);
